...
const Recent_Anime = () => {
  ...
  const loadRecentAnimes = async (pageNumber, isLoadMore = false) => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < pagesToLoad; i++) {
        ...
        const data = await fetchWithCORS(getApiUrl(`/category/recently-updated?page=${currentPage}`));
  ...
  const handleLoadMore = () => {
    ...
    setPage(nextPage);
    loadRecentAnimes(nextPage, true);
  };
